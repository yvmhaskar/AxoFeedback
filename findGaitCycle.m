...
%% Left Leg
    ...
    for j = 1:(size(struct.hs.tL,2)-1)
        ...
        stepsL{j} = struct.channel{ch}.filtered(start_idx:stop_idx);
...
%% Right Leg
    ...
    for j = 1:(size(struct.hs.tR,2)-1)
        ...
        stop_idx = stop - floor(samplerate * time_last_step(1));
        stepsR{j} = struct.channel{ch+ch_num/2}.filtered(start_idx:stop_idx);
     
        step_lengthR(j) = stop_idx-start_idx + 1;

     end
        stepL.trial = stepsL;
        struct.channel{ch}.stepL = stepL;
        stepR.trial = stepsR;
        struct.channel{ch+ch_num/2}.stepR = stepR;
