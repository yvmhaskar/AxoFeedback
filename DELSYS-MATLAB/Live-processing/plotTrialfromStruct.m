...
if strcmp(setting.type,'avg')
    ...
    for tr = tr_idx
        j = j +1;
        avg_data = squeeze(mean(struct.interp.data{tr},2));
        std_data = squeeze(std(struct.interp.data{tr},0,2));
        for ch = 1:ch_num
             ax = subplot(1,ch_num,ch_pos(ch));
             t= (0:size(avg_data(ch,:),2)-1)/size(avg_data(ch,:),2);
             plt(j) = plot(t,avg_data(ch,:),'Color',colors{j},'LineWidth',2);
             hold on
             xline(0.40)
             inBetween = [avg_data(ch,:) + std_data(ch,:),fliplr(avg_data(ch,:) - std_data(ch,:))];
             x_fill = [t,fliplr(t)];
