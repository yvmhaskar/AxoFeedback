...
date_key = "081522";
global EMGSaveFile
EMGSaveFile = subject_no + "_emg_struct_" + date_key;
global IMUSaveFile
IMUSaveFile = subject_no + "_imu_struct_" + date_key;
...
Muscle = ["Left GAS", "Left TA", "Left SOL","Right TA","Right GAS","Right SOL"];
for i=NUM_SENSORS:-1:1
    channel{i}.samplerate = 1926;
    channel{i}.muscle = Muscle(i);
end
emg_struct_raw.channel = channel;

sen = 6;
for i=NUM_SENSORS*3:-1:1
    channel{i}.samplerate = 148;
    channel{i}.Muscle = Muscle(sen);
    if i/3<=sen-1
        sen = sen-1;
    end
end
imu_struct_raw.channel = channel;
%% Saving file
save(EMGSaveFile,"emg_struct_raw")
save(IMUSaveFile,"imu_struct_raw")
...
if size(rawEMG,2)>10000
    disp("6 seconds of raw EMG data obtained")
    load(EMGSaveFile,"emg_struct_raw")
    for i=size(rawEMG,1):-1:1
        
        %channel{i} = emg_struct_raw.channel(i);
        if isfield(emg_struct_raw.channel(i),'raw_emg')
            emg = emg_struct_raw.channel(i).raw_emg;
        else
            emg = [];
        end
        channel{i}.raw_emg = [emg rawEMG(i,:)];
        channel{i}.samplerate = 1926;
        channel{i}.muscle = Muscle(i);
        emgData{i}.raw_emg = rawEMG(i,:);
        emgData{i}.samplerate = 1926;
        emgData{i}.muscle = Muscle(i);
        emg_struct_raw.channel{i} = channel(i);
    ...
    if size(rawIMU,2)>800
        disp("6 seconds of raw IMU data obtained")
        
   % Arranging IMU data: 
        load(IMUSaveFile,"imu_struct_raw")
        sen = 6;
        for i=size(rawIMU,1):-1:1
            
            %channel = imu_struct_raw.channel(i);
            if isfield(imu_struct_raw.channel,'raw_imu')
                imu = imu_struct_raw.channel.raw_emg;
            else
                imu = [];
            end
            channel{i}.raw_imu = [imu rawIMU(i,:)];
            channel{i}.samplerate = 148;
            channel{i}.Muscle = Muscle(sen);
            IMUData{i}.raw_imu = rawIMU(i,1:end);
            IMUData{i}.samplerate = 148;
            IMUData{i}.Muscle = Muscle(sen);
            
            if i/3<=sen-1
                sen = sen-1;            
            end
            imu_struct_raw.channel{i} = channel(i);
        end
...
%% Recorder on/off
        ...
        if recorder == 1
                save(EMGSaveFile,"emg_struct_raw")  % Date format: MMDDYY
                save(IMUSaveFile,"imu_struct_raw")
        end
%% Filtering IMU Data
...
% Change the pathname to the normalizing file
        maxfname = '1max_vol_contraction_081522';
